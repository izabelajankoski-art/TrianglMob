...
useEffect(() => {
    const normalized = attendances
        ...
        .filter(
            (a) =>
                filterType === "" ||
                a.class_session.course.type === filterType
        )
        .map((a) => ({
            attendance_id: a.id,
...
Object.entries(scheduleMap).forEach(([day, list]) => {
    const hasHighlighted = list.some(
        (c) =>
            c.class_session_id === highlightedSessionId
    );
...
const handleChangeStatus = async (
    ...
    if (Moment(sessionDate).isBefore(today)) {
        Alert.alert(
            "Zabrana",
            "Ne možete menjati status za prošle časove."
        );
    ...
    if (
        ...
        Alert.alert(
            "Zabrana",
            "Čas za danas ne može da se otkaže."
        );
    ...
    try {
        await api.post(
            "/attendance/change-confirmation-status",
            {
...
const selectedClasses = scheduleMap[selectedDate] || [];
...
return (
    <SafeAreaView style={tw`flex-1 bg-gray-100`}>
        ...
        {/* FILTER */}
        <View
            style={tw`flex-row justify-around py-2 bg-white`}
        >
        ...
        {/* KALENDAR */}
        <View
            style={tw`mt-4 mx-4 bg-white rounded-xl shadow`}
        >
            <Calendar
                onDayPress={(day) =>
        ...
        {/* ČASOVI - UOKVIRENA SCROLL SEKCIJA */}
        <View
            style={tw`
                    flex-1
                    mx-4
            ...
            >
                ...
                <Text
                    style={tw`
                        ...
                        `}
                >
                    Prevucite za pregled časova
                </Text>
            </View>

            {/* SCROLLUJE SE SAMO SADRŽAJ SA ČASOVIMA */}
            <ScrollView
